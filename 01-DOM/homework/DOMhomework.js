...
// Tu código acá:
  let toDoItems = [];
  let creadoPor = document.getElementById('createdBy')
  creadoPor.innerText += " " + "Santiago";
...
function buildToDo(todo, index) {
  ...
  // toDoText.addEventListener('click', evento => {
  //   completeToDo(evento);
  // })
...
function buildToDos(toDos) {
  // Tu código acá:
  let arrayMap = toDos.map((objeto, i) => buildToDo(objeto, i));
  return arrayMap;
}
...
function displayToDos() {
  ...
  resultado.forEach(valor => toDoContainer.appendChild(valor));
